...
// X3SpcPlayer is now a minimal SPC-image loader: it allocates a 64KB
...
typedef struct X3SpcPlayer {
  SpcPlayer base;
  uint8 ram[65536];
} X3SpcPlayer;

static void Dsp_Write(X3SpcPlayer *p, uint8_t reg, uint8 value) {
  if (p->base.dsp)
...
static void Spc_Reset(X3SpcPlayer *p) {
...
static void SmwSpcPlayer_Initialize(SpcPlayer *p_in) {
  X3SpcPlayer *p = (X3SpcPlayer *)p_in;
...
SpcPlayer *SmwSpcPlayer_Create(void) {
  X3SpcPlayer *p = (X3SpcPlayer *)malloc(sizeof(X3SpcPlayer));
  memset(p, 0, sizeof(X3SpcPlayer));
