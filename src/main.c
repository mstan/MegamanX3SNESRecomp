 ...
 * -DX3_RECOMP_UI=ON; its recomp_ui.cmake defines RECOMP_LAUNCHER. MMX drives
...
/* Game identity. Single region ((USA)) — no sibling variant target.
 * The launcher verifies the staged ROM against both digests below. */
#define X3_WINDOW_TITLE    "Mega Man X3 (Recompiled)"
#define X3_GAME_NAME       "Mega Man X3"
#define X3_GAME_REGION     "(USA)"
#define X3_LAUNCHER_TITLE  "Mega Man X3 \xE2\x80\x94 Launcher"
#define X3_ROM_CRC32       0xFA0FE671u
...
static const char kWindowTitle[] = X3_WINDOW_TITLE;
...
static void X3Display_PreparePpuFrame(void) {
  ...
  int width = X3Display_ComputeFrameWidth(drawable_width, drawable_height,
                                           g_config.widescreen);
  g_snes_width = width;
   ...
   * keyed to MMX1 WRAM; Mega Man X3 needs its own survey before any of
...
bool X3Display_IsWidescreenEnabled(void) { return g_config.widescreen; }
...
void ChangeWindowScale(int scale_step) {
  ...
  if (SDL_GetDisplayUsableBounds(screen, &bounds) == 0) {
    ...
    int logical_width = X3Display_GetWindowBaseWidth(g_snes_width);
    int logical_height = X3Display_GetWindowBaseHeight();
  ...
  int w = new_scale * X3Display_GetWindowBaseWidth(g_snes_width);
  int h = new_scale * X3Display_GetWindowBaseHeight();
...
static uint16_t x3_runner_to_snes_joypad(uint16_t r) {
  uint16_t s = 0;
...
static void SdlRenderer_BeginDraw(int width, int height, uint8 **pixels, int *pitch) {
  ...
  X3Display_ComputeViewport(width, height, output_width, output_height,
...
int main(int argc, char** argv) {
  ...
  host_report_init(X3_GAME_NAME " " X3_GAME_REGION, SNESRECOMP_BUILD_VERSION);
  ...
  static char rom_path_buf[512];
  {
    ...
    static const uint8_t kX3RomSha256[32] = {
     ...
     * positional ROM, or explicit environment override. */
    {
      ...
      if (want_launcher) {
        ...
        memset(&gi, 0, sizeof(gi));
#endif
        gi.name = X3_GAME_NAME;
        gi.region = X3_GAME_REGION;
        gi.sram_path = NULL;           /* hide SAVES panel — no battery SRAM
                                          (header ramSize=0, chips=ROM+COPRO);
                                          progress is carried by passwords. */
        gi.expected_crc = X3_ROM_CRC32;
    ...
    if (!snesrecomp_launcher_resolve_rom_sha256(la_argc, la_argv, rom_path_buf,
                                                sizeof(rom_path_buf), kX3RomSha256)) {
  ...
  // Initialize debug server
  {
     ...
     * 4384 Mega Man X3. Distinct ports let sibling games run
     * concurrently on one host without TCP-bind collisions. */
    if (debug_server_init(4384) == 0) {
      fprintf(stderr, "[main] Debug server ready on port 4384\n");
    }
    if (start_paused) {
  ...
  g_snes_width = g_config.widescreen
      ? X3Display_ComputeFrameWidth(16, 9, true) : 256;
  ...
  int window_width = custom_size ? g_config.window_width :
      g_current_window_scale * X3Display_GetWindowBaseWidth(g_snes_width);
  int window_height = custom_size ? g_config.window_height :
      g_current_window_scale * X3Display_GetWindowBaseHeight();
  ...
  extern const RtlGameInfo kX3GameInfo;
  RtlRegisterGame(&kX3GameInfo);
...
error_reading:;
  ...
  while (running) {
    ...
    // use; the disabled path is only the save-state repros).
    {
      ...
      emu_oracle_run_frame(x3_runner_to_snes_joypad((uint16_t)(inputs & 0xFFF)),
                           x3_runner_to_snes_joypad((uint16_t)((inputs >> 12) & 0xFFF)));
    }
#endif
    ...
    if (!g_snes->disableRender) {
      ...
      /* Turbo (render skipped): the per-scanline pass (X3DrawPpuFrame /
...
static void HandleCommand(uint32 j, bool pressed) {
  ...
  if (j <= kKeys_Load_Last) {
    ...
    case kKeys_ToggleWidescreen:
      X3Display_SetWidescreenEnabled(!g_config.widescreen);
      break;
    case kKeys_VolumeUp:
